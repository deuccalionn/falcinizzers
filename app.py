...
try:
    ...
    st.error(f"KÜTÜPHANE HATASI: requirements.txt dosyasında eksik var!\nHata: {e}")
    st.stop()
...
<style>
    ...
    .mystic-card { background: rgba(255,255,255,0.1); border: 1px solid #FFD700; border-radius: 15px; padding: 20px; margin-top: 20px; text-align: center; }
    .stButton>button { background: linear-gradient(90deg, #6a11cb 0%, #2575fc 100%); color: white; border-radius: 25px; width: 100%; margin-top: 15px; }
</style>
...
with st.sidebar:
    ...
    if 'GOOGLE_API_KEY' in st.secrets:
        ...
        st.success("Yapay Zeka Hazır 🟢")
    else:
        st.error("API Key Bulunamadı! Secrets ayarlarını kontrol et.")
        api_key = st.text_input("Manuel Key Gir", type="password")

# MODELİ BAŞLAT (HATA VARSA SÖYLE)
model = None
if api_key:
    try:
        ...
        st.error(f"MODEL BAŞLATMA HATASI: {e}")
...
with tab1:
    ...
    if uploaded_file and st.button("KAHVE FALIMA BAK"):
        if not model:
            st.error("Yapay Zeka Modeli yüklenemedi. Lütfen API Key'i kontrol et.")
        else:
            image = Image.open(uploaded_file)
            st.image(image, width=300)
            with st.spinner("Yorumlanıyor..."):
                try:
                    prompt = f"Falcı ol. Ad: {isim}. Fincanı yorumla. Mistik ve 3 paragraf olsun."
                    res = model.generate_content([prompt, image])
                    st.markdown(f'<div class="mystic-card">{res.text}</div>', unsafe_allow_html=True)
                except Exception as e:
                    st.error(f"FAL BAKILIRKEN HATA OLUŞTU: {e}")
